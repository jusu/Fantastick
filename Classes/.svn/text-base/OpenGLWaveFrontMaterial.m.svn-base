			...
			for (int j = i; j <= mtlEnd; j++)
			{
				...
				else if ([parseLine hasPrefix:@"map_Kd "])
				{
					...
					for (int i=4; i <= 1028; i*=2)
					{
						...
						NSString *newBase = [NSString stringWithFormat:@"%@-%d.pvr4", baseName, i];
						NSString *path = [FSCache stringWithModelDirectory: newBase];
						//[[NSBundle mainBundle] pathForResource:newBase ofType:@"pvr4"];
						if([FSCache isCached: path])
						{
							textureFilename = [NSString stringWithFormat:@"%@", newBase];
							width = i;
							height = i;
							break;
						}
						newBase = [NSString stringWithFormat:@"%@-%d.pvr2", baseName, i];
						path = [FSCache stringWithModelDirectory: newBase];
						//[[NSBundle mainBundle] pathForResource:newBase ofType:@"pvr2"];
						if([FSCache isCached: path])
						{
							textureFilename = [NSString stringWithFormat:@"%@", newBase];
							width = i;
